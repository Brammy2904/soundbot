...
export class SoundsCommand extends ConfigCommand {
  ...
  public run(message: Message, params: string[]) {
    ...
    if (!sounds.length) {
      message.channel.send(localize.t('commands.sounds.notFound', { prefix: this.config.prefix }));
      return;
    }

    const page = parseInt(params[0]);
    chunkedMessages(sounds, page).forEach(chunk => message.channel.send("Current sounds: "+chunk));
  }
}
